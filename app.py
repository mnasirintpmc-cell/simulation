...
def find_controlling_valve_for_pipe(pipe_index, valves, valve_states):
    ...
    # SPECIAL CASE: V-104 always controls pipe 22 regardless of distance
    if pipe_number == 22 and "V-104" in valves:
        # Show distance for debugging
        v104_data = valves["V-104"]
        v104_distance = ((v104_data["x"] - x1)**2 + (v104_data["y"] - y1)**2)**0.5
        if pipe_index == 21:  # Only show for actual pipe 22
            st.sidebar.info(f"V-104 to Pipe 22 distance: {v104_distance:.1f}px")
        return "V-104"
...
def get_pipe_color_based_on_leader_system(pipe_index, valves, valve_states):
    ...
    # DEBUG: For pipe 22, show detailed information
    if pipe_number == 22:
        debug_info = []
        ...
        for group_name, group_data in pipe_groups.items():
            if pipe_number == group_data["leader"]:
                debug_info.append(f"Leader of {group_name}")
                controlling_valve = find_controlling_valve_for_pipe(pipe_index, valves, valve_states)
                debug_info.append(f"Controlling valve: {controlling_valve}")
                debug_info.append(f"Valve state: {valve_states.get(controlling_valve, False)}")
        
        # Check if it's a follower
        for group_name, group_data in pipe_groups.items():
            if pipe_number in group_data["followers"]:
                debug_info.append(f"Follower in {group_name}")
                leader_pipe = group_data["leader"]
                debug_info.append(f"Follows Pipe {leader_pipe}")
                leader_controlling_valve = find_controlling_valve_for_pipe(leader_pipe-1, valves, valve_states)
                debug_info.append(f"Leader's controlling valve: {leader_controlling_valve}")
                debug_info.append(f"Leader's valve state: {valve_states.get(leader_controlling_valve, False)}")
        
        if debug_info:
            st.sidebar.error(f"Pipe 22 Debug: {' | '.join(debug_info)}")
...
with col2:
    ...
    **Workflow:**
    ...
    4. **V-104 SPECIAL**: Always controls Pipe 22 regardless of distance
