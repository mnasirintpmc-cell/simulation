...
# ===================== PRESSURE SOURCES =====================
# CHANGE THESE to match your system (pipes connected to pumps, headers, tanks, etc.)
PRESSURE_SOURCES = [1, 5, 11]  # ← Edit this line! Example: main inlet, after pump, utility header
...
def get_pipe_color(idx_0):
    ...
    # Has pressure? (from sources + propagation through open paths)
    ...
    if not has_pressure:
        for leader_1, followers in get_groups().items():
            ...
            if leader_idx in active_leaders and (num == leader_1 or num in followers):
                if leader_1 in PRESSURE_SOURCES or any(src == leader_1 for src in PRESSURE_SOURCES):
                    has_pressure = True
                    break
...
with st.sidebar:
    ...
    for tag in valves:
        ...
        button_label = f"{'OPEN' if current_state else 'CLOSED'} {tag}"
    ...
    for i in range(len(st.session_state.pipes)):
        ...
        status_icon = "Selected" if is_selected else "Pipe"
        if st.button(f"{status_icon} {i+1}", key=f"pipe_{i}", use_container_width=True):
            st.session_state.selected_pipe = i
            st.rerun()

    st.markdown("---")
    st.header("Pipe Position Adjustment")
    if st.session_state.selected_pipe is not None:
        ...
        with col1:
            new_x1 = st.number_input("X1", value=pipe["x1"], key="x1_adj")
            new_y1 = st.number_input("Y1", value=pipe["y1"], key="y1_adj")
        with col2:
            new_x2 = st.number_input("X2", value=pipe["x2"], key="x2_adj")
            new_y2 = st.number_input("Y2", value=pipe["y2"], key="y2_adj")
        if st.button("Apply Changes", use_container_width=True):
            st.session_state.pipes[st.session_state.selected_pipe].update({"x1": new_x1, "y1": new_y1, "x2": new_x2, "y2": new_y2})
...
with col2:
    st.header("Flow & Pressure Status")
    st.markdown("---")
    ...
    st.write(f"**Pressurized:** {pressurized}")
    st.write(f"**Empty:** {len(pipes) - pressurized}")

st.success("Realistic pressure simulation added! Change PRESSURE_SOURCES = [1, 5, 11] to match your system.")import streamlit as st
import json
from PIL import Image, ImageDraw
import os
import math
