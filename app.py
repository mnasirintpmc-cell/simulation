...
st.title("🧠 Interactive P&ID Valve Simulator")
...
try:
    ...
    st.error(f"❌ Could not find '{PID_FILE}'. Please upload it to the repository root.")
    st.stop()

try:
    valve_icon = Image.open(VALVE_ICON_FILE).convert("RGBA")
except FileNotFoundError:
    st.error(f"❌ Could not find '{VALVE_ICON_FILE}'. Please upload it to the repository root.")
    st.stop()
...
st.write("🔍 Detecting valve tags using OCR...")
...
for tag, x, y, w, h in detected_tags:
    if tag not in unique_tags:
        unique_tags[tag] = (x, y, w, h)

detected_tags = [(tag, *vals) for tag, vals in unique_tags.items()]

if not detected_tags:
    st.warning("⚠️ No valve tags detected. Try lowering the confidence threshold or use a clearer P&ID image.")
else:
    st.success(f"✅ {len(detected_tags)} unique valve tag(s) detected successfully.")
...
for tag, x, y, w, h in detected_tags:
    ...
    valve_states[tag] = st.sidebar.toggle(f"{tag}", value=False, key=f"toggle_{tag}")
